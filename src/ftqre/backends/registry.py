...
if TYPE_CHECKING:
    from ftqre.backends.base import LogicalEstimator, PhysicalEstimator
...
def discover_logical_estimators() -> dict[str, LogicalEstimator]:
    ...
    from ftqre.backends.mock import MockLogicalEstimator
    ...
    try:
        ...
        eps = entry_points(group="ftqre.logical_estimators")
        for ep in eps:
...
def discover_physical_estimators() -> dict[str, PhysicalEstimator]:
    ...
    from ftqre.backends.mock import AnalyticalPhysicalEstimator
    ...
    try:
        ...
        eps = entry_points(group="ftqre.physical_estimators")
        for ep in eps:
            if ep.name not in estimators:
                try:
                    cls = ep.load()
