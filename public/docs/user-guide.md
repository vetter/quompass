...
For a high-rate qLDPC code with `qubits_formula` constant in `d` and
`transversal_magic_states=True`, the estimator drops the 15-to-1 factory
entirely and the resulting physical-qubit count can be orders of magnitude
below the same circuit on a surface code.
